...
namespace DocumentsKM.Services
{
    ...
    {
        private IMarkGeneralDataRepo _repository;

        public GeneralDataDocService(IMarkGeneralDataRepo markGeneralDataRepo)
        {
            _repository = markGeneralDataRepo;
        }

        public async Task<MemoryStream> GetDocByMarkId(int markId)
        {
            var markGeneralData = _repository.GetByMarkId(markId);
            var memory = new MemoryStream();
            var docStr = @"\documentclass[12pt, a4paper]{article}
...
\begin{enumerate}[label*=\arabic*]";
            ...
            foreach (var section in markGeneralData.Sections)
            {
                docStr += $"\\item {section.Title}\n";
