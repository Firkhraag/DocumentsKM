...
namespace DocumentsKM.Controllers
{
    ...
    public class MarkGeneralDataPointsController : ControllerBase
    {
        ...
        [HttpPatch, Route("marks/{markId}/general-data-sections/{sectionId}/general-data-points")]
        [Consumes(MediaTypeNames.Application.Json)]
        // [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public ActionResult<IEnumerable<MarkGeneralDataPointResponse>> UpdateAllByPointIds(
            int markId, int sectionId, [FromBody] List<int> pointIds)
        {
            try
            {
                var addedPoints = _service.UpdateAllByPointIds(markId, sectionId, pointIds);
